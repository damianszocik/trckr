...
import { addTrackerData, editTrackerEntry } from '../../state/state';
  ...
  return (
   <Form layout="vertical" hideRequiredMark onSubmit={this.handleFormSubmit}>
    <Form.Item label="Entry time" {...formItemLayout} validateStatus={timeFieldError ? 'error' : ''}>
     {this.props.form.getFieldDecorator('time', {
      ...
      <DatePicker
       ...
       onChange={moment => this.setState({ dateTime: moment ? moment : null })}
       onOk={moment => this.setState({ dateTime: moment ? moment : null })}
      />
     )}
    </Form.Item>
    <Form.Item label="Value" {...formItemLayout} validateStatus={valueFieldError ? 'error' : ''} help={valueFieldError ? 'Please, provide entry value.' : ''}>
     {this.state.tracker.options.trackerType == 'binary' && (
      <Binary
       icons={this.state.tracker.options.binaryIcons}
       initialVal={this.props.editedEntry ? this.props.editedEntry.entryData.value : null}
       emitEntryValue={this.setEntryValue}
      />
     )}
     {this.state.tracker.options.trackerType == 'rating' && (
      <Rating
       range={this.state.tracker.options.ratingRange}
       initialVal={this.props.editedEntry ? this.props.editedEntry.entryData.value : null}
       emitEntryValue={this.setEntryValue}
      />
     )}
     {this.state.tracker.options.trackerType == 'custom' && (
      <Custom initialVal={this.props.editedEntry ? this.props.editedEntry.entryData.value : null} emitEntryValue={this.setEntryValue} />
     )}
    </Form.Item>
    <Form.Item label="Notes" {...formItemLayout}>
     <Input.TextArea
      defaultValue={this.props.editedEntry ? this.props.editedEntry.entryData.note : null}
      rows={2}
